...
async function execute(inputItems, runQueries) {
    ...
    for (let i = 0; i < inputItems.length; i++) {
        ...
        if (outputColumns.includes('*')) {
            query = `${SELECT} * FROM ${(0, utils_1.escapeSqlIdentifier)(table)}`;
        }
        else {
            const escapedColumns = outputColumns.map(utils_1.escapeSqlIdentifier).join(', ');
            query = `${SELECT} ${escapedColumns} FROM ${(0, utils_1.escapeSqlIdentifier)(table)}`;
        }
        let values = [];
