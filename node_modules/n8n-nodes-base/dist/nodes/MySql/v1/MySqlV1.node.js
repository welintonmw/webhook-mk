    ...
    async execute() {
        ...
        if (operation === 'executeQuery') {
            try {
                const queryQueue = items.map(async (_, index) => {
                    ...
                    for (const resolvable of (0, utilities_1.getResolvables)(rawQuery)) {
                        rawQuery = rawQuery.replace(resolvable, this.evaluateExpression(resolvable, index));
                    }
            ...
            try {
                ...
                const queryQueue = updateItems.map(async (item) => await connection.query(updateSQL, Object.values(item).concat(item[updateKey])));
