...
function parseDateByFormat(value, fromFormat) {
    ...
    if ((0, moment_timezone_1.default)(date).isValid())
        return date;
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Date input cannot be parsed. Please recheck the value and the "From Format" field.');
}
function getIsoValue(value) {
    try {
        ...
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Unrecognized date input. Please specify a format in the "From Format" field.');
    }
}
...
const versionDescription = {
    ...
    properties: [
        {
            displayName: "More powerful date functionality is available in <a href='https://docs.n8n.io/code-examples/expressions/luxon/' target='_blank'>expressions</a>,</br> e.g. <code>{{ $now.plus(1, 'week') }}</code>",
            name: 'noticeDateTime',
            type: 'notice',
            default: '',
        },
    ...
    async execute() {
        ...
        for (let i = 0; i < length; i++) {
            try {
                ...
                item = items[i];
                if (action === 'format') {
                    let currentDate = this.getNodeParameter('value', i);
                    ...
                    if (options.fromFormat === undefined && !(0, moment_timezone_1.default)(currentDate).isValid()) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'The date input format could not be recognized. Please set the "From Format" field', { itemIndex: i });
                    }
                    if (Number.isInteger(currentDate)) {
                    ...
                    (0, set_1.default)(newItem, ['json', dataPropertyName], newDate);
                ...
                if (action === 'calculate') {
                    ...
                    (0, set_1.default)(newItem, ['json', dataPropertyName], newDate.toISOString());
                    returnData.push(newItem);
                }
            }
            catch (error) {
