    ...
    constructor() {
        ...
        this.methods = {
            loadOptions: {
                async getHashes() {
                    const returnData = [];
                    const hashes = crypto_1.getHashes();
        ...
        for (let i = 0; i < length; i++) {
            try {
                ...
                if (action === 'hmac') {
                    ...
                    const encoding = this.getNodeParameter('encoding', i);
                    newValue = crypto_1.createHmac(type, secret).update(value).digest(encoding);
                }
                if (action === 'sign') {
                    const algorithm = this.getNodeParameter('algorithm', i);
                    const encoding = this.getNodeParameter('encoding', i);
                    const privateKey = this.getNodeParameter('privateKey', i);
                    const sign = crypto_1.createSign(algorithm);
                    sign.write(value);
                    sign.end();
                ...
                lodash_1.set(newItem, `json.${dataPropertyName}`, newValue);
