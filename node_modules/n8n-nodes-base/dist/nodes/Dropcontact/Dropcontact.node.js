    ...
    async execute() {
        ...
        if (resource === 'contact') {
            if (operation === 'enrich') {
                ...
                responseData = (await GenericFunction_1.dropcontactApiRequest.call(this, 'POST', '/batch', { data, siren, language }, {}));
                if (!responseData.success) {
                    if (this.continueOnFail()) {
                        const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ error: responseData.reason || 'invalid request' }), {
                            itemData: (0, GenericFunction_1.mapPairedItemsFrom)(entryData),
                        });
                ...
                if (!simplify) {
                    const waitTime = this.getNodeParameter('options.waitTime', 0, 45);
                    const delay = async (ms) => await new Promise((res) => setTimeout(res, ms * 1000));
                    await delay(waitTime);
                    responseData = await GenericFunction_1.dropcontactApiRequest.call(this, 'GET', `/batch/${responseData.request_id}`, {}, {});
                    if (!responseData.success) {
                        if (this.continueOnFail()) {
                            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ error: responseData.reason }), {
                                itemData: (0, GenericFunction_1.mapPairedItemsFrom)(entryData),
                            });
                            returnData.push(...executionData);
                        }
                        ...
                        responseData.data.forEach((d, index) => {
                            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(d), { itemData: { item: index } });
                            returnData.push(...executionData);
                        });
                    }
                }
            ...
            if (operation === 'fetchRequest') {
                for (let i = 0; i < entryData.length; i++) {
                    ...
                    responseData = (await GenericFunction_1.dropcontactApiRequest.call(this, 'GET', `/batch/${requestId}`, {}, {}));
                    if (!responseData.success) {
                        if (this.continueOnFail()) {
                            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ error: responseData.reason || 'invalid request' }), { itemData: { item: i } });
                            returnData.push(...executionData);
                        }
                        else {
                            throw new n8n_workflow_1.NodeApiError(this.getNode(), {
...
exports.Dropcontact = Dropcontact;
//# sourceMappingURL=Dropcontact.node.js.map