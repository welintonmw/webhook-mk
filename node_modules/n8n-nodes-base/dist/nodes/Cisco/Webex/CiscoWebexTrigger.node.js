...
class CiscoWebexTrigger {
        ...
        this.webhookMethods = {
            default: {
                async checkExists() {
                    ...
                    for (const webhook of data) {
                        if (webhook.url === webhookUrl
                            && webhook.resource === GenericFunctions_1.mapResource(resource)
                            && webhook.event === event
                ...
                async create() {
                    ...
                    const secret = GenericFunctions_1.getAutomaticSecret(credentials);
                    const filter = [];
                    ...
                    const body = {
                        ...
                        event,
                        resource: GenericFunctions_1.mapResource(resource),
                    };
                    if (filters.ownedBy) {
    ...
    async webhook() {
        ...
        const computedSignature = crypto_1.createHmac('sha1', webhookData.secret).update(req.rawBody).digest('hex');
