...
class GithubTrigger {
    constructor() {
        ...
        this.webhookMethods = {
            default: {
                async checkExists() {
                    ...
                    try {
                        await GenericFunctions_1.githubApiRequest.call(this, 'GET', endpoint, {});
                    }
                    catch (error) {
                ...
                async create() {
                    ...
                    let responseData;
                    try {
                        responseData = await GenericFunctions_1.githubApiRequest.call(this, 'POST', endpoint, body);
                    }
                    catch (error) {
                        if (error.cause.httpCode === '422') {
                            responseData = await GenericFunctions_1.githubApiRequest.call(this, 'GET', endpoint, body);
                            for (const webhook of responseData) {
                                if (webhook.config.url === webhookUrl) {
                                    if (JSON.stringify(webhook.events) === JSON.stringify(events)) {
                                        webhookData.webhookId = webhook.id;
                                        webhookData.webhookEvents = webhook.events;
                                        return true;
                                    }
                                }
                            }
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'A webhook with the identical URL probably exists already. Please delete it manually on Github!', { level: 'warning' });
                        }
                        if (error.cause.httpCode === '404') {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Check that the repository exists and that you have permission to create the webhooks this node requires', { level: 'warning' });
                        }
                        throw error;
                    }
                ...
                async delete() {
                    ...
                    if (webhookData.webhookId !== undefined) {
                        ...
                        const body = {};
                        try {
                            await GenericFunctions_1.githubApiRequest.call(this, 'DELETE', endpoint, body);
                        }
                        catch (error) {
                            return false;
                        }
        ...
        this.methods = {
            listSearch: {
                getUsers: SearchFunctions_1.getUsers,
                getRepositories: SearchFunctions_1.getRepositories,
            },
        };
    }
    async webhook() {
