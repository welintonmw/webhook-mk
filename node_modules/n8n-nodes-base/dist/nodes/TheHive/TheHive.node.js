        ...
        this.methods = {
            loadOptions: {
                async loadResponders() {
                    const resource = GenericFunctions_1.mapResource(this.getNodeParameter('resource'));
                ...
                async loadObservableOptions() {
                    var _a;
                    const version = (_a = (await this.getCredentials('theHiveApi'))) === null || _a === void 0 ? void 0 : _a.apiVersion;
                    const options = [
            ...
            try {
                if (resource === 'alert') {
                    if (operation === 'count') {
                        ...
                        const countQueryAttributs = GenericFunctions_1.prepareOptional(filters);
                        const _countSearchQuery = QueryFunctions_1.And();
                        if ('customFieldsUi' in filters) {
                            const customFields = await GenericFunctions_1.prepareCustomFields.call(this, filters);
                            const searchQueries = GenericFunctions_1.buildCustomFieldSearch(customFields);
                            _countSearchQuery['_and'].push(...searchQueries);
                        }
                        for (const key of Object.keys(countQueryAttributs)) {
                            if (key === 'tags') {
                                _countSearchQuery['_and'].push(QueryFunctions_1.In(key, countQueryAttributs[key]));
                            }
                            else if (key === 'description' || key === 'title') {
                                _countSearchQuery['_and'].push(QueryFunctions_1.ContainsString(key, countQueryAttributs[key]));
                            }
                            else {
                                _countSearchQuery['_and'].push(QueryFunctions_1.Eq(key, countQueryAttributs[key]));
                            }
                        }
                        const body = {
                            'query': [
                    ...
                    if (operation === 'get') {
                        const alertId = this.getNodeParameter('id', i);
                        responseData = await GenericFunctions_1.theHiveApiRequest.call(this, 'GET', `/alert/${alertId}`, {});
                    }
                    if (operation === 'getAll') {
                        const credentials = await this.getCredentials('theHiveApi');
                        ...
                        const version = credentials.apiVersion;
                        const filters = this.getNodeParameter('filters', i, {});
                        const queryAttributs = GenericFunctions_1.prepareOptional(filters);
                        const options = this.getNodeParameter('options', i);
                        const _searchQuery = QueryFunctions_1.And();
                        if ('customFieldsUi' in filters) {
                            const customFields = await GenericFunctions_1.prepareCustomFields.call(this, filters);
                            const searchQueries = GenericFunctions_1.buildCustomFieldSearch(customFields);
                        ...
                        for (const key of Object.keys(queryAttributs)) {
                            if (key === 'tags') {
                                _searchQuery['_and'].push(QueryFunctions_1.In(key, queryAttributs[key]));
                            }
                            else if (key === 'description' || key === 'title') {
                                _searchQuery['_and'].push(QueryFunctions_1.ContainsString(key, queryAttributs[key]));
                            }
                            else {
                                _searchQuery['_and'].push(QueryFunctions_1.Eq(key, queryAttributs[key]));
                            }
                        }
                        let endpoint;
                        let method;
                        ...
                        if (version === 'v1') {
                            ...
                            GenericFunctions_1.prepareSortQuery(options.sort, body);
                            if (limit !== undefined) {
                                GenericFunctions_1.prepareRangeQuery(`0-${limit}`, body);
                            }
                            qs.name = 'alerts';
                        }
                        else {
                            ...
                            Object.assign(qs, GenericFunctions_1.prepareOptional(options));
                        }
                ...
                if (resource === 'observable') {
                    if (operation === 'count') {
                        const countQueryAttributs = GenericFunctions_1.prepareOptional(this.getNodeParameter('filters', i, {}));
                        const _countSearchQuery = QueryFunctions_1.And();
                        for (const key of Object.keys(countQueryAttributs)) {
                            if (key === 'dataType' || key === 'tags') {
                                ...
                                _countSearchQuery['_and'].push(QueryFunctions_1.Between('startDate', countQueryAttributs['range']['dateRange']['fromDate'], countQueryAttributs['range']['dateRange']['toDate']));
                            }
                            else {
                                _countSearchQuery['_and'].push(QueryFunctions_1.Eq(key, countQueryAttributs[key]));
                            }
                    ...
                    if (operation === 'create') {
                        const caseId = this.getNodeParameter('caseId', i);
                        let body = Object.assign({ dataType: this.getNodeParameter('dataType', i), message: this.getNodeParameter('message', i), startDate: Date.parse(this.getNodeParameter('startDate', i)), tlp: this.getNodeParameter('tlp', i), ioc: this.getNodeParameter('ioc', i), sighted: this.getNodeParameter('sighted', i), status: this.getNodeParameter('status', i) }, GenericFunctions_1.prepareOptional(this.getNodeParameter('options', i, {})));
                        let options = {};
                    ...
                    if (operation === 'getAll') {
                        ...
                        if (version === 'v1') {
                            ...
                            body.query = QueryFunctions_1.Parent('case', QueryFunctions_1.Id(caseId));
                            Object.assign(qs, GenericFunctions_1.prepareOptional(options));
                        }
                        responseData = await GenericFunctions_1.theHiveApiRequest.call(this, method, endpoint, body, qs);
                    }
                    if (operation === 'search') {
                        ...
                        const returnAll = this.getNodeParameter('returnAll', i);
                        const version = credentials.apiVersion;
                        const queryAttributs = GenericFunctions_1.prepareOptional(this.getNodeParameter('filters', i, {}));
                        const _searchQuery = QueryFunctions_1.And();
                        const options = this.getNodeParameter('options', i);
                        for (const key of Object.keys(queryAttributs)) {
                            if (key === 'dataType' || key === 'tags') {
                                ...
                                _searchQuery['_and'].push(QueryFunctions_1.Between('startDate', queryAttributs['range']['dateRange']['fromDate'], queryAttributs['range']['dateRange']['toDate']));
                            }
                            else {
                                _searchQuery['_and'].push(QueryFunctions_1.Eq(key, queryAttributs[key]));
                            }
                    ...
                    if (operation === 'update') {
                        ...
                        const body = Object.assign({}, GenericFunctions_1.prepareOptional(this.getNodeParameter('updateFields', i, {})));
                ...
                if (resource === 'case') {
                    ...
                    if (operation === 'create') {
                        ...
                        const body = Object.assign({ title: this.getNodeParameter('title', i), description: this.getNodeParameter('description', i), severity: this.getNodeParameter('severity', i), startDate: Date.parse(this.getNodeParameter('startDate', i)), owner: this.getNodeParameter('owner', i), flag: this.getNodeParameter('flag', i), tlp: this.getNodeParameter('tlp', i), tags: GenericFunctions_1.splitTags(this.getNodeParameter('tags', i)), customFields }, GenericFunctions_1.prepareOptional(options));
                    ...
                    if (operation === 'update') {
                        ...
                        const jsonParameters = this.getNodeParameter('jsonParameters', i);
                        const customFields = await GenericFunctions_1.prepareCustomFields.call(this, updateFields, jsonParameters);
                        const body = Object.assign({ customFields }, GenericFunctions_1.prepareOptional(updateFields));
                ...
                if (resource === 'task') {
                    ...
                    if (operation === 'create') {
                        const caseId = this.getNodeParameter('caseId', i);
                        const body = Object.assign({ title: this.getNodeParameter('title', i), status: this.getNodeParameter('status', i), flag: this.getNodeParameter('flag', i) }, GenericFunctions_1.prepareOptional(this.getNodeParameter('options', i, {})));
                    ...
                    if (operation === 'getAll') {
                        ...
                        if (version === 'v1') {
                            ...
                            body.query = QueryFunctions_1.And(QueryFunctions_1.Parent('case', QueryFunctions_1.Id(caseId)));
                            Object.assign(qs, GenericFunctions_1.prepareOptional(options));
                        }
                        responseData = await GenericFunctions_1.theHiveApiRequest.call(this, method, endpoint, body, qs);
                    }
                ...
                if (resource === 'log') {
                    ...
                    if (operation === 'getAll') {
                        ...
                        if (version === 'v1') {
                            ...
                            body.query = QueryFunctions_1.And(QueryFunctions_1.Parent('task', QueryFunctions_1.Id(taskId)));
                        }
                        responseData = await GenericFunctions_1.theHiveApiRequest.call(this, method, endpoint, body, qs);
                    }
                }
