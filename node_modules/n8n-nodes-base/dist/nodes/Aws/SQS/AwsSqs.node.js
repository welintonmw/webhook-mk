    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                const params = [
                    'Version=2012-11-05',
                    `Action=${change_case_1.pascalCase(operation)}`,
                ];
                ...
                this.getNodeParameter('options.messageAttributes.binary', i, []).forEach((attribute) => {
                    ...
                    if (item.binary === undefined) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No binary data set. So message attribute cannot be added!');
                    }
                    if (item.binary[dataPropertyName] === undefined) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The binary property "${dataPropertyName}" does not exist. So message attribute cannot be added!`);
                    }
