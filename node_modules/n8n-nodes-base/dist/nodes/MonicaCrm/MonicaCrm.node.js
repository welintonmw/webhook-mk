        ...
        this.methods = {
            loadOptions: {
                async getActivityTypes() {
                    ...
                    return GenericFunctions_1.toOptions(responseData);
                },
                async getTagsToAdd() {
            ...
            try {
                if (resource === 'activity') {
                    ...
                    if (operation === 'create') {
                        ...
                        const _a = this.getNodeParameter('additionalFields', i), { isDeceased = false, deceasedDate, birthdate } = _a, rest = __rest(_a, ["isDeceased", "deceasedDate", "birthdate"]);
                        body.is_birthdate_known = false;
                        body.is_deceased = isDeceased;
                        body.is_deceased_date_known = false;
                        if (birthdate) {
                            body.is_birthdate_known = true;
                            const [day, month, year] = GenericFunctions_1.getDateParts(birthdate);
                        ...
                        if (deceasedDate) {
                            ...
                            const [day, month, year] = GenericFunctions_1.getDateParts(deceasedDate);
                        ...
                        const _b = this.getNodeParameter('updateFields', i), { is_deaceased = false, deceased_date, birthdate } = _b, rest = __rest(_b, ["is_deaceased", "deceased_date", "birthdate"]);
                        body.is_birthdate_known = false;
                        ...
                        if (deceased_date) {
                            body.is_deceased = true;
                            body.is_deceased_date_known = true;
                            const [day, month, year] = GenericFunctions_1.getDateParts(deceased_date);
                            body.deceased_date_day = day;
                            body.deceased_date_month = month;
                            body.deceased_date_year = year;
                        }
                    ...
                    if (operation === 'add') {
                        ...
                        const updateFields = this.getNodeParameter('updateFields', i, {});
                        const { data } = await GenericFunctions_1.monicaCrmApiRequest.call(this, 'GET', `/conversations/${conversationId}`);
                        const message = data.messages.filter((message) => message.id === parseInt(messageId, 10))[0];
                        const body = {
                            contact_id: data.contact.id,
