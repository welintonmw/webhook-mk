...
exports.SlackTrigger = void 0;
const GenericFunctions_1 = require("./V2/GenericFunctions");
const SlackTriggerHelpers_1 = require("./SlackTriggerHelpers");
class SlackTrigger {
    constructor() {
        this.description = {
                ...
                },
                {
                    ...
                    options: [
                        ...
                        },
                        {
                            ...
                            default: [],
                            description: 'A comma-separated string of encoded user IDs. Choose from the list, or specify IDs using an <a href="https://docs.n8n.io/code-examples/expressions/">expression</a>.',
                        },
                    ],
                },
            ],
    ...
    async webhook() {
        ...
        if (!filters.includes('file_share') &&
            !filters.includes('any_event') &&
            !filters.includes(eventType)) {
            return {};
        }
        const eventChannel = (_a = req.body.event.channel) !== null && _a !== void 0 ? _a : req.body.event.item.channel;
        if (!watchWorkspace) {
        ...
        if (options.resolveIds) {
            if (req.body.event.user) {
                if (req.body.event.type === 'reaction_added') {
                    req.body.event.user_resolved = await SlackTriggerHelpers_1.getUserInfo.call(this, req.body.event.user);
                    req.body.event.item_user_resolved = await SlackTriggerHelpers_1.getUserInfo.call(this, req.body.event.item_user);
                }
                else {
                    req.body.event.user_resolved = await SlackTriggerHelpers_1.getUserInfo.call(this, req.body.event.user);
                }
            ...
            if (eventChannel) {
                const channel = await SlackTriggerHelpers_1.getChannelInfo.call(this, eventChannel);
        ...
        if (req.body.event.subtype === 'file_share' &&
            ...
            if (this.getNodeParameter('downloadFiles', false)) {
                for (let i = 0; i < req.body.event.files.length; i++) {
                    const file = (await SlackTriggerHelpers_1.downloadFile.call(this, req.body.event.files[i].url_private_download));
                    binaryData[`file_${i}`] = await this.helpers.prepareBinaryData(file, req.body.event.files[i].name, req.body.event.files[i].mimetype);
                }
            }
        }
        return {
...
exports.SlackTrigger = SlackTrigger;
