...
class ScheduleTrigger {
    ...
    async trigger() {
        ...
        const fallbackToZero = (0, GenericFunctions_1.addFallbackValue)(nodeVersion >= 1.2, '0');
        const executeTrigger = async (recurency) => {
            const resultData = {
                timestamp: moment_timezone_1.default.tz(timezone).toISOString(true),
                'Readable date': moment_timezone_1.default.tz(timezone).format('MMMM Do YYYY, h:mm:ss a'),
                'Readable time': moment_timezone_1.default.tz(timezone).format('h:mm:ss a'),
                'Day of week': moment_timezone_1.default.tz(timezone).format('dddd'),
                Year: moment_timezone_1.default.tz(timezone).format('YYYY'),
                Month: moment_timezone_1.default.tz(timezone).format('MMMM'),
                'Day of month': moment_timezone_1.default.tz(timezone).format('DD'),
                Hour: moment_timezone_1.default.tz(timezone).format('HH'),
                Minute: moment_timezone_1.default.tz(timezone).format('mm'),
                Second: moment_timezone_1.default.tz(timezone).format('ss'),
                Timezone: moment_timezone_1.default.tz(timezone).format('z Z'),
            };
        ...
        for (let i = 0; i < interval.length; i++) {
            ...
            if (interval[i].field === 'cronExpression') {
                if (nodeVersion > 1) {
                    (0, GenericFunctions_1.convertToUnixFormat)(interval[i]);
                }
                const cronExpression = interval[i].expression;
                try {
                    const cronJob = new cron_1.CronJob(cronExpression, async () => await executeTrigger({ activated: false }), undefined, true, timezone);
            ...
            if (interval[i].field === 'seconds') {
                ...
                const intervalObj = setInterval(async () => await executeTrigger({ activated: false }), intervalValue);
            ...
            if (interval[i].field === 'hours') {
                ...
                const minute = fallbackToZero((_a = interval[i].triggerAtMinute) === null || _a === void 0 ? void 0 : _a.toString());
                ...
                if (hour === 1) {
                    ...
                    const cronJob = new cron_1.CronJob(cronExpression, async () => await executeTrigger({
                        activated: true,
                        index: i,
                        intervalSize: hour,
                        typeInterval: 'hours',
            ...
            if (interval[i].field === 'days') {
                ...
                const minute = fallbackToZero((_c = interval[i].triggerAtMinute) === null || _c === void 0 ? void 0 : _c.toString());
            ...
            if (interval[i].field === 'weeks') {
                ...
                const minute = fallbackToZero((_e = interval[i].triggerAtMinute) === null || _e === void 0 ? void 0 : _e.toString());
            ...
            if (interval[i].field === 'months') {
                ...
                const minute = fallbackToZero((_h = interval[i].triggerAtMinute) === null || _h === void 0 ? void 0 : _h.toString());
                const cronTimes = [minute, hour, day, '*', '*'];
                const cronExpression = cronTimes.join(' ');
                if (month === 1) {
                    const cronJob = new cron_1.CronJob(cronExpression, async () => await executeTrigger({ activated: false }), undefined, true, timezone);
