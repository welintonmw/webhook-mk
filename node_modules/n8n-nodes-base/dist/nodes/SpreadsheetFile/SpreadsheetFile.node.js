    ...
    async execute() {
        ...
        if (operation === 'fromFile') {
            ...
            for (let i = 0; i < items.length; i++) {
                try {
                    ...
                    if (options.readAsString === true) {
                        workbook = xlsx_1.read(binaryData.toString(), { type: 'string', raw: options.rawData });
                    }
                    else {
                        workbook = xlsx_1.read(binaryData, { raw: options.rawData });
                    }
                    ...
                    if (options.headerRow === false) {
                        for (const rowData of sheetJson) {
                            newItems.push({ json: { row: rowData } });
                        }
                    }
                    else {
                        for (const rowData of sheetJson) {
                            newItems.push({ json: rowData });
                        }
                    }
                }
                catch (error) {
            ...
            try {
                ...
                const wbout = xlsx_1.write(wb, wopts);
                const newItem = {
                    json: {},
                    binary: {},
                };
