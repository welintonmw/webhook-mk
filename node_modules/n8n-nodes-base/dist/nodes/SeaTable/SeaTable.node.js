            ...
            if (operation === 'create') {
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    try {
                        if (fieldsToSend === 'autoMapInputData') {
                            ...
                            const inputDataToIgnore = GenericFunctions_1.split(this.getNodeParameter('inputsToIgnore', i, ''));
                        ...
                        body.row = GenericFunctions_1.rowExport(rowInput, GenericFunctions_1.updateAble(tableColumns));
                        ...
                        const newRowInsertData = GenericFunctions_1.rowMapKeyToName(responseData, tableColumns);
                        ...
                        const row = GenericFunctions_1.rowFormatColumns(Object.assign(Object.assign({}, newRowInsertData), newRow), tableColumns.map(({ name }) => name).concat(['_id', '_ctime', '_mtime']));
                        returnData.push(row);
                    }
                    catch (error) {
                ...
                try {
                    for (let i = 0; i < items.length; i++) {
                        ...
                        const rows = responseData.map((row) => GenericFunctions_1.rowFormatColumns(Object.assign({}, row), tableColumns.map(({ name }) => name).concat(['_id', '_ctime', '_mtime'])));
                ...
                for (let i = 0; i < items.length; i++) {
                    try {
                        ...
                        const response = await GenericFunctions_1.seaTableApiRequest.call(this, ctx, 'DELETE', `/dtable-server/api/v1/dtables/{{dtable_uuid}}/rows/`, body, qs);
                        returnData.push(response);
                    }
                    catch (error) {
                        if (this.continueOnFail()) {
