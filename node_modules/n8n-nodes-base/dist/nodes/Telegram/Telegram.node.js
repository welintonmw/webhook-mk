...
class Telegram {
    constructor() {
        ...
        this.methods = {
            credentialTest: {
                async telegramBotTest(credential) {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (binaryData === true) {
                    ...
                    const propertyName = GenericFunctions_1.getPropertyName(operation);
                    const formData = Object.assign(Object.assign({}, body), { [propertyName]: {
                            value: dataBuffer,
                            options: {
                ...
                if (resource === 'file' && operation === 'get') {
                    if (this.getNodeParameter('download', i, false) === true) {
                        ...
                        const file = await GenericFunctions_1.apiRequest.call(this, 'GET', '', {}, {}, { json: false, encoding: null, uri: `https://api.telegram.org/file/bot${credentials.accessToken}/${filePath}`, resolveWithFullResponse: true });
                        const fileName = filePath.split('/').pop();
                        const binaryData = await this.helpers.prepareBinaryData(Buffer.from(file.body), fileName);
                        returnData.push({
                            json: responseData,
                            binary: {
                                data: binaryData,
