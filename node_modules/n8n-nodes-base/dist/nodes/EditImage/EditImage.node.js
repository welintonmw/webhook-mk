...
class EditImage {
    constructor() {
        ...
        this.methods = {
            loadOptions: {
                async getFonts() {
                    const files = await (0, get_system_fonts_1.default)();
                    const returnData = [];
                    files.forEach((entry) => {
                        const pathParts = (0, path_1.parse)(entry);
                        if (!pathParts.ext) {
                            return;
                        }
                        returnData.push({
            ...
            try {
                ...
                for (let i = 0; i < operations.length; i++) {
                    ...
                    if (operationData.operation === 'blur') {
                        ...
                        await (0, promises_1.writeFile)(path, binaryDataBuffer);
                        if (operations[0].operation === 'create') {
                            gmInstance = (0, gm_1.default)(gmInstance.stream('png'))
                                .compose(operator)
                ...
                if (item.binary !== undefined && newItem.binary) {
                    ...
                    if (newItem.binary[dataPropertyName]) {
                        newItem.binary[dataPropertyName] = (0, n8n_workflow_1.deepCopy)(newItem.binary[dataPropertyName]);
                    }
                }
