...
class CrateDb {
    ...
    async execute() {
        ...
        if (operation === 'executeQuery') {
            const queryResult = await Postgres_node_functions_1.pgQuery(this.getNodeParameter, pgp, db, items, this.continueOnFail());
            returnItems = this.helpers.returnJsonArray(queryResult);
        }
        else if (operation === 'insert') {
            const insertData = await Postgres_node_functions_1.pgInsert(this.getNodeParameter, pgp, db, items, this.continueOnFail());
            ...
            if (mode === 'independently') {
                const updateItems = await Postgres_node_functions_1.pgUpdate(this.getNodeParameter, pgp, db, items, this.continueOnFail());
                ...
                const returning = Postgres_node_functions_1.generateReturning(pgp, this.getNodeParameter('returnFields', 0));
                const queries = [];
                for (let i = 0; i < items.length; i++) {
                    const itemCopy = Postgres_node_functions_1.getItemCopy(items[i], columns);
                ...
                returnItems = this.helpers.returnJsonArray(Postgres_node_functions_1.getItemsCopy(items, columns));
            }
        }
        else {
            await pgp.end();
