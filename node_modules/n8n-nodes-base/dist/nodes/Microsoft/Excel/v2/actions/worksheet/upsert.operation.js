...
async function execute(items) {
    ...
    try {
        ...
        (0, utils_1.checkRange)(this.getNode(), range);
        ...
        if (dataMode === 'raw') {
            ...
            responseData = await transport_1.microsoftApiRequest.call(this, 'PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, { values });
        }
        if (dataMode !== 'raw' &&
            (worksheetData.values === undefined || worksheetData.values.length <= 1)) {
        ...
        const appendAfterSelectedRange = this.getNodeParameter('options.appendAfterSelectedRange', 0, false);
        if (nodeVersion > 2 && !appendAfterSelectedRange && updateSummary.updatedData.length) {
            for (let i = updateSummary.updatedData.length - 1; i >= 0; i--) {
                if (updateSummary.updatedData[i].every((item) => item === '' || item === undefined || item === null)) {
        ...
        if (updateSummary.appendData.length) {
            ...
            if (nodeVersion > 2 && !appendAfterSelectedRange) {
                const { address } = await transport_1.microsoftApiRequest.call(this, 'GET', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/usedRange`, undefined, { select: 'address' });
