        ...
        this.methods = {
            credentialTest: {
                async microsoftSqlConnectionTest(credential) {
                    ...
                    try {
                        const pool = (0, GenericFunctions_1.configurePool)(credentials);
                        await pool.connect();
                    }
                    catch (error) {
                        return {
    ...
    async execute() {
        ...
        try {
            ...
            if (operation === 'insert') {
                ...
                await (0, GenericFunctions_1.insertOperation)(tables, pool);
                responseData = items;
            }
            if (operation === 'update') {
                const updateKeys = items.map((_, index) => this.getNodeParameter('updateKey', index));
                const tables = (0, GenericFunctions_1.createTableStruct)(this.getNodeParameter, items, ['updateKey'].concat(updateKeys), 'updateKey');
                await (0, GenericFunctions_1.updateOperation)(tables, pool);
                responseData = items;
            }
            if (operation === 'delete') {
                ...
                responseData = await (0, GenericFunctions_1.deleteOperation)(tables, pool);
            }
            const itemData = (0, utilities_1.generatePairedItemData)(items.length);
            returnData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(responseData), { itemData });
        }
        catch (error) {
            if (this.continueOnFail(error)) {
                responseData = items;
